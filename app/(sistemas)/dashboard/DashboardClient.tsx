...
export default function DashboardClient() {
  ...
  useEffect(() => {
    async function init() {
      ...
      const [{ data: emp }, { data: orcsNovos }, { data: orcsPendentes }] = await Promise.all([
        supabase.from('empresas_detail').select('*').eq('id', usuario.empresa_id).single(),
      ...
      setTotais(prev => ({ ...prev, orcamentosPendentes: orcsPendentes?.length || 0 }))
      setLoading(false)
    }
    init()
  }, [])
  ...
  const cards = [
    { label: 'OS ABERTAS', icon: '🔧', value: String(totais.osAbertas), sub: 'ordens em aberto', cor: '#D4A843', gradiente: 'linear-gradient(90deg, #D4A843, #F0C060)' },
    { label: 'ORÇAMENTOS', icon: '📋', value: String(totais.orcamentosPendentes), sub: 'aguardando aprovação', cor: '#90CDF4', gradiente: 'linear-gradient(90deg, #2B6CB0, #90CDF4)' },
    { label: 'AGENDA HOJE', icon: '📅', value: String(totais.agendamentosHoje), sub: 'agendamentos', cor: '#48BB78', gradiente: 'linear-gradient(90deg, #276749, #48BB78)' },
    { label: 'FATURAMENTO', icon: '💰', value: 'R$ 0,00', sub: 'no mês atual', cor: '#CBD5E0', gradiente: 'linear-gradient(90deg, #4A5568, #CBD5E0)' },
  ]
